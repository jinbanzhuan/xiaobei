    ...
    def test_beta(self):
        ...
        for cycle in range(1, 3):
            ...
            try:
                ...
                assert get_checklist.status_code == 200, f"\n[03] 🙅 新增走访失败,响应码错误:{get_checklist.status_code}"
                ...
                while True:
                    ...
                    if get_result.json()['data']['result'] != '':
                        self.logger.info(
                            f"[06] ✅ 当前接口状态 {get_result.json()['data']['status']},第 {cycle} 次 ♻️循环 - 报告分析成功,♻️循环 {attempt} 次, ⏳ 耗时 {elapsed_time:.2f}秒")
                        break
                ...
                self.logger.info(f"========== 异步任务状态轮询 第 {cycle} 次 ♻️ 循环结束 ==========\n")
                ...
                assert visitings.json()['data'][
                           'status'] == "visiting", f"\n[08] 🙅 状态非checklist,实际状态:{visitings.json()['data']['status']}"
                ...
                assert confirmed.json()['data'][
                           'status'] == "confirmed", f"\n[09] 🙅 状态非checklist,实际状态:{confirmed.json()['data']['status']}"
                ...
                while True:
                    ...
                    if parse_result.json()['status'] == "completed":
                        self.logger.info(
                            f"[12] ✅ 当前接口状态 {parse_result.json()['status']},第 {cycle} 次 ♻️循环 - 报告分析成功,♻️循环 {attempts} 次, ⏳ 耗时 {elapsed_time:.2f}秒")
                        break
                ...
                self.logger.info(f"========== 妙计解析任务状态 第 {cycle} 次 ♻️ 循环结束 ==========\n")
                ...
                while True:
                    ...
                    if get_tasks.json()["data"]["status"] == "completed":
                        self.logger.info(
                            f"[14] ✅ 当前接口状态 {get_tasks.json()['data']['status']},第 {cycle} 次 ♻️循环 - 报告分析成功,♻️循环 {attemptss} 次, ⏳ 耗时 {elapsed_time:.2f}秒")
                        break
